...
function spotClicked(event) {
  ...
  if (event.target.innerHTML == "X" || event.target.innerHTML == "0") {
    ...
    event.target.style.color = "#298340";
    event.target.style.fontSize = "100px";
    event.target.style.justifyContent = "center";
    count = count + 1;
  } else {
    event.target.innerHTML = "O";
    event.target.style.color = "#047593";
    count = count + 1;
    // event.target.innerHTML = "O";
    event.target.style.fontSize = "100px";
    event.target.style.justifyContent = "center";
  }
    //place X's and O's on the table when spot is clicked
...
function resetGame() {
  ...
  //each spot is set to empty string
  for (i = 0; i < spot.length; i++) {
    spot[i].addEventListener("click", spotClicked);
    spot[i].style.background = "black";
    spot[i].innerHTML = " ";
  }
  winner.innerHTML = " ";
  resetButton.style.display = "none";
